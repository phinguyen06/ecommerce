...
public class OrderDB {
	 ...
	 */
	protected int updateCustomer(Connection conn, Customer customer)
			throws ServiceException {
	...
	public int createOrder(OrderWoodProduct[] orderWoodProducts)
		...
		try {
			...
			if(  orderWoodProducts[0].getOrder().getCustomer().getCustomerId() > 0 ){
				customerId = updateCustomer(conn, orderWoodProducts[0].getOrder().getCustomer());
				isUpdate = true;
			}else
				customerId = insertCustomer(conn, orderWoodProducts[0].getOrder().getCustomer());
			...
			if( isUpdate )
				udb.updateUser(conn, user);
			else
				udb.insertUser(conn, user);
			
			//STEP 6: Create Shipping requirement
			ShipDB sdb = new ShipDB();
			sdb.insertShipping(conn, order.getShipping());
