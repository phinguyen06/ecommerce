...
public class WoodDB {
	...
	static final String s_SQL_QUERY_ALL_WOOD_PRODUCT =
		...
		"			wp.shipping_cost, wp.image_name, wp.quantity, wp.vendor_id, wp.vendor_name, " +
		...
		throws ServiceException
	{
		try {
			...
			{
				//System.out.println("working on category:"+categories[i]);
				List<WoodProduct> prods = (List<WoodProduct>) sqlMap.queryForList("getHotProduct", categories[i]);
				//System.out.println("result:"+prods.size());
				for( WoodProduct prod : prods){
					list.add(prod);
				}
			}
	 ...
	 * @throws ServiceException
	 */
	public static WoodProduct[] getHotWoodProduct2(int[] categories)
			throws ServiceException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Connection conn = null;
		try {
			...
			for( int i=0; i<categories.length; i++ )
			{
				...
				while (rs.next()) {
					...
					product.setQuantity(rs.getInt(index++));
	...
	/**
	 * getAllWoodProduct - Lookup all wood products with category
	 * @return WoodProduct[] - Array of Wood Product in the system
	 * @throws ServiceException
	 */
	@SuppressWarnings("unchecked")
			...
			throws ServiceException
	{
		try {
			...
			SqlMapClient sqlMap = SqlConfig.getInstance().getSqlMap();
			//System.out.println("sqlMap:"+sqlMap);
			List<WoodProduct> prods = (List<WoodProduct>) sqlMap.queryForList("getAllWoodProduct");
			WoodProduct[] rets = new WoodProduct[prods.size()];
			int i=0;
			for( WoodProduct prod : prods){
				rets[i++] = prod;
			}
	 ...
	 * @throws ServiceException
	 */
	public static WoodProduct[] getAllWoodProduct2()
			throws ServiceException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Connection conn = null;
			...
			throws ServiceException
	{
		try {
			...
			SqlMapClient sqlMap = SqlConfig.getInstance().getSqlMap();
			List<WoodProduct> prods = (List<WoodProduct>) sqlMap.queryForList("getProductByCategory", categoryId);
			WoodProduct[] rets = new WoodProduct[prods.size()];
			int i=0;
			for( WoodProduct prod : prods){
				rets[i++] = prod;
	 ...
	 * @throws ServiceException
	 */
	public static WoodProduct[] getWoodProduct2(int categoryId)
			throws ServiceException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Connection conn = null;
			...
			throws ServiceException 
	{
		try {
			...
			SqlMapClient sqlMap = SqlConfig.getInstance().getSqlMap();
			List<WoodProduct> prods = (List<WoodProduct>) sqlMap.queryForList("getProductByIds", list);
			System.out.println("result size:"+prods.size());
			WoodProduct[] rets = new WoodProduct[prods.size()];
	 ...
	 * @throws ServiceException
	 */
	public static WoodProduct[] getWoodProductByIds2(int[] ids)
			throws ServiceException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Connection conn = null;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static OrderWoodProduct[] getOrderWoodProducts2(int orderId)
			throws ServiceException
	{
		try {
			logger.debug("WoodDB::getOrderWoodProducts:START");
			SqlMapClient sqlMap = SqlConfig.getInstance().getSqlMap();
			List<OrderWoodProduct> prods = (List<OrderWoodProduct>) sqlMap.queryForList("getOrderedProduct", orderId);
			OrderWoodProduct[] rets = new OrderWoodProduct[prods.size()];
			int i=0;
			for( OrderWoodProduct prod : prods){
				rets[i++] = prod;
			}
			return rets;
	...
	public static OrderWoodProduct[] getOrderWoodProducts(int orderId)
		...
		try {
			...
			while(rs.next()){
				...
				wp.setQuantity(rs.getInt(index++));
	...
	/**
	 * updateProductQuantity - Update product inventory quantity
	 * @param product - Product to update 
	 * @throws ServiceException
	 */
	public void updateProductQuantity(WoodProduct product)
			throws ServiceException
	{
		try {
			logger.debug("WoodDB::updateProductQuantity:START");
			SqlMapClient sqlMap = SqlConfig.getInstance().getSqlMap();
			sqlMap.update("updateProductQuantity", product);
		} catch (SQLException sqle) {
			sqle.printStackTrace();
			throw new ServiceException(sqle);
		} catch (Exception e) {
			...
			logger.debug("WoodDB::updateProductQuantity:END");
		}
	...
	public static void main(String[] args) throws Exception
	{
		...
		*/
		...
		WoodProduct[] prods = WoodDB.getHotWoodProduct(categories);
		for( int i=0; i<prods.length; i++ ){
			System.out.println("["+i+"] prods["+prods[i]+"] name["+prods[i].getName()+"]");
			logger.debug("["+i+"] prods["+prods[i]+"] name["+prods[i].getName()+"]");
		}
		...
		*/
		...
		int[] ids = new int[]{1,3,170};
		WoodProduct[] prods = WoodDB.getWoodProductByIds(ids);
		for( int i=0; i<prods.length; i++ ){
			logger.debug("["+i+"] prods["+prods[i]+"] name["+prods[i].getName()+"] imageURL["+prods[i].getImageURL()+"]");
			System.out.println("["+i+"] prods["+prods[i]+"] name["+prods[i].getName()+"] imageURL["+prods[i].getImageURL()+"]");
		}
		*/
		...
		OrderWoodProduct[] orders = WoodDB.getOrderWoodProducts(20);
		...
		*/
		...
		/*
		logger.debug("-------update product quantity for productId["+prods[0].getProductId()+"] --------------");
		System.out.println("-------update product quantity for productId["+prods[0].getProductId()+"] --------------");
		prods[0].setQuantity(1);
		WoodDB db = new WoodDB();
		db.updateProductQuantity(prods[0]);
		*/
