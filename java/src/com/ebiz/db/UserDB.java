...
public class UserDB {
			...
			throws ServiceSecurityException, ServiceException 
	{
		try {
			...
			List<User> usrs = (List<User>) sqlMap.queryForList("validateUser", email);
			if( usrs != null && usrs.size() > 0)
			{			
				User user = usrs.get(0);
				String password = EcryptoDataProvider.getInstance().getEcrypto().decrypt(user.getPassword());
				...
				if( Constants.s_WEB_USER_PROFILE.equals(user.getPermission().getCode()) ){
	 ...
	 * @throws ServiceException
	 */
	public static User validateUser2(String email, String pwd, boolean isValidate)
			throws ServiceSecurityException, ServiceException {
	...
	public static User validateAdminUser(String email, String pwd)
		...
		try {
			...
			List<User> usrs = (List<User>) sqlMap.queryForList("validateAdminUser", email);
			if( usrs != null && usrs.size() > 0)
			{			
				User user = usrs.get(0);
				String password = EcryptoDataProvider.getInstance().getEcrypto().decrypt(user.getPassword());
	 ...
	 * @throws ServiceException
	 */
	public static User validateAdminUser2(String email, String pwd)
			throws ServiceSecurityException, ServiceException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Connection conn = null;
	 ...
	 * @throws ServiceException
	 */
	public void insertUser(SqlMapClient sqlMap, User user)
			throws ServiceException{ 
		try {
			logger.debug("UserDB::insertUser:START");
			//Encrypt the plain user password before store in DB
			user.setPassword(EcryptoDataProvider.getInstance().getEcrypto().encrypt(user.getPassword()));
	 ...
	 * @throws ServiceException
	 */
	public void insertUser2(Connection conn, User user)
			throws ServiceException {
		PreparedStatement pstmt = null;
		try {
			logger.debug("UserDB::insertUser:START");
	 ...
	 * @throws ServiceException
	 */
	public void updateUser2(Connection conn, User user)
			throws ServiceException {
		PreparedStatement pstmt = null;
		try {
			logger.debug("UserDB::updateUser:START");
